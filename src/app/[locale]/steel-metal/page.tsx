...
function SubcategoryCard({
  ...
  category: SteelMetalSubcategory;
  index: number;
      ...
      >
        ...
        {category.alibaba_aliases.length > 0 && (
          <p className="mt-2 text-sm text-ink-muted">
            {alsoCalledLabel}: {category.alibaba_aliases.slice(0, 3).join(" · ")}
          </p>
        )}
...
export default async function SteelMetalHubPage() {
  ...
  const millLeafCount = mill.reduce((sum, cat) => sum + cat.children.length, 0);
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="container-page relative">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3 text-sm">
            <span className="rounded-full border border-white/15 bg-white/5 px-3 py-1 font-medium text-white">
              {t("statSubcategories", { count: mill.length })}
            </span>
            <span className="rounded-full border border-white/15 bg-white/5 px-3 py-1 font-medium text-white">
              {t("statLeaves", { count: millLeafCount })}
            </span>
      ...
      <div className="container-page py-12">
        <ol className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {mill.map((category, index) => (
            <SubcategoryCard
              key={category.id}
              category={category}
              index={index + 1}
              alsoCalledLabel={t("alsoCalled")}
              leafCountLabel={t("leafCount", { count: category.children.length })}
              browseLabel={t("browseTypes", { count: category.children.length })}
            />
          ))}
        </ol>
        ...
        {more.length > 0 && (
          <section className="mt-14 border-t border-slate-200 pt-10">
            <p className="eyebrow text-ink-dim">{t("moreMetals")}</p>
            <p className="mt-2 max-w-2xl text-sm text-ink-muted">{t("moreMetalsHint")}</p>
