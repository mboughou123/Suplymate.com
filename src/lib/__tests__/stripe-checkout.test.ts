...
describe("subscription Checkout Session params", () => {
  ...
  it("omits trial_period_days when the caller passes 0", () => {
  ...
  it("includes a 3-day trial for Enterprise the same as other paid plans", () => {
    const params = buildSubscriptionCheckoutParams(
      { ...base, plan: "enterprise", trialDays: 3, priceId: "price_ent" },
      { integrationSuffix: "enttrial1" },
    );
    expect(params.subscription_data?.trial_period_days).toBe(3);
  });
});
