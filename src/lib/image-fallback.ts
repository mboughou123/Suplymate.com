...
// and must never win over a local still. Other remotes (Vercel Blob, mill
// sites) are used only when no local still exists. Swap in preferred photos
// any time by replacing the SVGs or pointing these maps at curated CDN URLs.
 ...
 * Wordmark / logo thumbs under `…/logos/` are not card stills.
 */
export function isLocalStillUrl(url?: string | null): boolean {
  if (typeof url !== "string") return false;
  const value = url.trim();
  if (/\/logos?\//i.test(value) || /logo[-_.]/i.test(value)) return false;
  return /^\/images\/(suppliers|products)\/.+\.(jpe?g|png|webp)$/i.test(value);
}
 ...
 *
 * Never prefer Google Maps URLs over local stills — Maps hosts 403 and 502
 * `/_next/image` on production.
 */
