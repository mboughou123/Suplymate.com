...
/**
 * Global primary chrome matching the homepage glass nav.
 * Used on homepage, suppliers, products, price charts, AI assistant, pricing.
 */
export default function SiteNav() {
  ...
  const requestDemo = () => {
    ...
    router.push("/#ai-demo-walkthrough");
  };
      ...
      >
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label={t("mainNav")}>
          {LINKS.map(({ href, key }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`rounded-lg px-3 py-2 text-sm font-medium transition hover:bg-slate-50 hover:text-ink ${
                  active ? "bg-slate-50 text-ink" : "text-ink-muted"
                }`}
              >
                {nav(key)}
              </Link>
          ...
          <Link
            href="/pricing"
            className={`rounded-lg px-3 py-2 text-sm font-medium transition hover:bg-slate-50 hover:text-ink ${
              pathname === "/pricing" ? "bg-slate-50 text-ink" : "text-ink-muted"
            }`}
          >
            {nav("pricing")}
          </Link>
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <button type="button" onClick={requestDemo} className="btn-accent px-4 py-2 text-sm">
            {t("requestDemo")}
          </button>
        </div>
      ...
      {open && (
        <div className="pointer-events-auto mx-auto mt-2 max-w-6xl rounded-2xl border border-white/80 bg-white/85 p-4 shadow-glass backdrop-blur-xl md:hidden">
          ...
          <div className="mt-3 flex flex-col gap-2 border-t border-slate-100 pt-3">
            <LanguageSelector />
            {session?.user ? (
              <Link href="/dashboard" className="btn-secondary justify-center py-2.5 text-sm">
                {nav("dashboard")}
              </Link>
            ) : (
            ...
            <button type="button" onClick={requestDemo} className="btn-accent justify-center py-2.5 text-sm">
              {t("requestDemo")}
            </button>
          </div>
        </div>
