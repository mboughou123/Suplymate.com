...
type Props = {
  /** Inner pages stay on the solid glass bar (homepage overlays the dark hero). */
  solid?: boolean;
};

export default function HomeTopNav({ solid = false }: Props) {
  ...
  const home = homeForRole((session?.user as { role?: unknown } | undefined)?.role);
