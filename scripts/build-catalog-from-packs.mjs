 ...
 *   data/daily-2026-09-17-mills-cleared.json (34 mills: day OK5+soft4 + HOLD30 OK14+soft11; jm-eagle + soft-hold11 + blocked4 out)
...
function loadDaily0915Seals() {
  const pack = readJsonIfExists(path.join(DATA_DIR, "daily-2026-09-15-products-cleared.json"));
  return { pack, productWire: productWireFromPack(pack) };
}

function buildDaily0915ProductHosts(wiredSlugs, report) {
  const seals = loadDaily0915Seals();
  return buildProductHostsFromPack({
    pack: seals.pack,
...
function loadHold5ProductSeals() {
  const pack = readJsonIfExists(path.join(DATA_DIR, "daily-2026-09-15-hold5-products-cleared.json"));
  return { pack, productWire: productWireFromPack(pack) };
}

function buildDaily0915Hold5ProductHosts(wiredSlugs, report) {
  const seals = loadHold5ProductSeals();
  return buildProductHostsFromPack({
    pack: seals.pack,
...
function loadProductPacks() {
  ...
  const day0915 = readJsonIfExists(path.join(DATA_DIR, "daily-2026-09-15-products-cleared.json"));
  if (day0915) {
    const seals = loadDaily0915Seals();
    const skus = (day0915.products ?? []).map(normalizeClearedSku).filter((sku) => {
      if (seals.productWire.size && !seals.productWire.has(sku.supplier_slug_guess)) return false;
      return true;
    });
    if (skus.length) {
      packs.push({
        packId: "d0915",
        scrapedAt: "2026-09-15T17:40:00.000Z",
        skus: skus.map((sku) => ({ raw: sku, bucket: "daily" })),
      });
    }
  }
  ...
  const hold5 = readJsonIfExists(path.join(DATA_DIR, "daily-2026-09-15-hold5-products-cleared.json"));
  if (hold5) {
    const seals = loadHold5ProductSeals();
    const skus = (hold5.products ?? []).map(normalizeClearedSku).filter((sku) => {
      if (seals.productWire.size && !seals.productWire.has(sku.supplier_slug_guess)) return false;
      return true;
    });
    if (skus.length) {
      packs.push({
        packId: "d0915-h5",
        scrapedAt: "2026-09-15T18:55:00.000Z",
        skus: skus.map((sku) => ({ raw: sku, bucket: "daily" })),
      });
    }
  }
...
export function buildCatalog() {
  ...
  const day0915Mills = buildAppendedStillsMills({
    file: "daily-2026-09-15-mills-cleared.json",
    packTag: "daily-2026-09-15",
    dateTag: "2026-09-15",
    skipSlugs: new Set([
      ...locked0911Slugs,
  ...
  const hold7Mills = buildAppendedStillsMills({
    file: "daily-2026-09-15-hold7-mills-cleared.json",
    packTag: "daily-2026-09-15-hold7",
    dateTag: "2026-09-15",
    skipSlugs: new Set([
      ...locked0911Slugs,
      ...skip12Mills.map((s) => s.packSlug),
