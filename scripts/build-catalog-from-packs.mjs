...
const HOLD30_FOREVER_OUT = new Set(["winpak", "superior-tube", "regal-rexnord", "rr-kabel"]);
...
/**
 * HOLD30 re-QA mill pack (`data/hold30-mills-cleared.json`).
 * Gated by `docs/researcher-hold30-mills-2026-09-10.json`. Appends the
 * Researcher-cleared 26 (OK + soft) without rewriting the locked daily
 * 2026-09-10 twenty. winpak / superior-tube / regal-rexnord stay HOLD;
 * rr-kabel stays out forever.
 */
function loadHold30Seals() {
  ...
  const softSlugs = new Set(
    [...(pack?.mills_soft ?? []), ...(mills?.soft ?? []).map(slugOf)].filter(
      (slug) => slug && millWire.has(slug)
    )
  );
